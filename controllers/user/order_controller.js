...
exports.orderAll = async (req, res) => {
  ...
  await connection.query(
    `SELECT * FROM histories WHERE id_user=?`,
    [id_user],
...
exports.orderPending = async (req, res) => {
  const id_user = req.decoded.id_user;
  await connection.query(
    `SELECT * FROM histories WHERE id_user=? AND status=0`,
    [id_user],
    function (error, rows, fields) {
      if (error) {
        console.log(error);
...
exports.orderProcess = async (req, res) => {
  const id_user = req.decoded.id_user;
  await connection.query(
    `SELECT * FROM histories WHERE id_user=? AND (status=3 OR status=4 OR status=5) ORDER BY ordered_at DESC`,
    [id_user],
    function (error, rows, fields) {
      if (error) {
        console.log(error);
...
exports.orderCBU = async (req, res) => {
  const id_user = req.decoded.id_user;
  await connection.query(
    `SELECT * FROM histories WHERE id_user=? AND status=1`,
    [id_user],
    function (error, rows, fields) {
      if (error) {
        console.log(error);
        ...
        if (rows.length == 0) {
          res.json({
            status: 204,
            message: "There's no order canceled by you",
          });
        } else if (rows.length > 0) {
          res.json({ status: 200, values: rows });
        }
...
exports.orderCBA = async (req, res) => {
  const id_user = req.decoded.id_user;
  await connection.query(
    `SELECT * FROM histories WHERE id_user=? AND status=2`,
    [id_user],
    function (error, rows, fields) {
      if (error) {
        console.log(error);
        ...
        if (rows.length == 0) {
          res.json({
            status: 204,
            message: "There's no order canceled by admin",
          });
        } else if (rows.length > 0) {
          res.json({ status: 200, values: rows });
        }
...
exports.orderPaid = async (req, res) => {
  const id_user = req.decoded.id_user;
  await connection.query(
    `SELECT * FROM histories WHERE id_user=? AND status=3`,
    [id_user],
    function (error, rows, fields) {
      if (error) {
        console.log(error);
...
exports.orderReady = async (req, res) => {
  const id_user = req.decoded.id_user;
  await connection.query(
    `SELECT * FROM histories WHERE id_user=? AND status=5`,
    [id_user],
    function (error, rows, fields) {
      if (error) {
        console.log(error);
...
exports.orderDone = async (req, res) => {
  const id_user = req.decoded.id_user;
  await connection.query(
    `SELECT * FROM histories WHERE id_user=? AND status=6`,
    [id_user],
    function (error, rows, fields) {
      if (error) {
        console.log(error);
                    ...
                    connection.query(`SELECT * FROM item_histories WHERE id_history=?`, [id_history],
                        function (error, rows, fields) {
                            if (error) {
                                ...
                                res.json({status: 200, values: {history: history, product: rows}});
                            }
                        }
                    );
                }
