...
exports.checkoutCart = async (req, res) => {
  ...
  connection.query(qValidationItem, [id_user], function (error, rows, result) {
    if (error) {
      console.log(error);
      return res
        .status(500)
      ...
      if (!validation_cart) {
        return res
          .status(400)
          .json({ status: 400, message: "Cannot checkout an empty cart" });
      } else {
        const qItemCart = `SELECT i.id_cart_item, i.id_product, i.amount, i.id_cart,
                                        ...
                                        i.id_cart=?`;
        connection.query(qItemCart, [id_user], function (error, rows, result) {
          if (error) {
            console.log(error);
            return res
              .status(500)
            ...
            for (let x = 0; x < rows.length; x++) {
              ...
              if (validateAmount > validateStock) {
                return res
                  ...
                  .json({
                    status: 400,
                    message: `Jumlah produk tidak memenuhi pesananmu`,
                  });
              }
            }
            ...
            connection.query(qHistory, vHistory, function (error, r, result) {
              if (error) {
                console.log(error);
                return res
                  .status(500)
                  ...
                  function (error, r, result) {
                    if (error) {
                      ...
                      for (let i = 0; i < loop; i++) {
                        ...
                        connection.query(
                          qItemHistory,
                          [iHistory, product_name, type, price, amount],
                            ...
                            connection.query(
                              ...
                              function (error, rrrr, result) {
                                if (error) {
                                  ...
                                  connection.query(
                                    `SELECT * FROM informations WHERE id_information=1`,
                                    (error, rr, result) => {
                                      if (error) {
                                        console.log(error);
                                        res
...
exports.confirmOrder = async (req, res) => {
  ...
  connection.query(
    `UPDATE histories SET status=3 WHERE id_history=?`,
    [id_history],
    function (error, rows, result) {
      if (error) {
        console.log(error);
...
exports.cancelOrder = async (req, res) => {
  ...
  const id_history = req.params.id_history;
  connection.query(
    `UPDATE histories SET status=1, finished_at=? WHERE id_history=?`,
    [date_time, id_history],
    function (error, rows, result) {
      if (error) {
        console.log(error);
        res.status(500).json({ status: 500, message: "Internal Server Error" });
